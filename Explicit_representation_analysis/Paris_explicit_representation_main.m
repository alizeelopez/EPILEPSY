...
if task==0
    dir_to_safe=[a_path '\eeg_rawdata_AL\Preproc_explicit'];
end
mkdir(dir_to_safe)
...
a_entfile =dir([a_path '\eeg_rawdata_AL\' '*.eeg.ent']);
a_entfile=[a_path '\eeg_rawdata_AL\' a_entfile.name];
...
%     else
a_pos_file=dir([dir_to_safe filesep  '*ds' int2str(s_downsamp) '.pos']);
a_pos=[dir_to_safe filesep a_pos_file.name];
