...
id_fix=find(m_pos(:,2)==50);%OK: 120 events
...
fix_latency_eeg=m_pos(id_fix,1);%
...
if length(id_cue)<120
    ...
    if length(id_fix)==120 %if we have all fixation: it's quite easy:
        id_trial_no_cue=find(~ismember(id_fix+1,id_cue));%identify trial number for which we did not have cue and pick up from logfile the relative latency of the stim event + resp event during this trial
...
id_fix=find( m_pos_corrected(:,2)==50);%OK: 120 events
